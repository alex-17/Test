package compress.lzma.Compression.LZMA;

import java.io.IOException;

import javax.print.attribute.standard.Compression;

import compress.lzma.ICodeProgress;
import compress.lzma.Compression.RangeCoder.BitTreeEncoder;
...
public class Encoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init() { compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(m_Encoders); }



			public void Encode(compress.lzma.Compression.RangeCoder.Encoder rangeEncoder, byte symbol) throws IOException
			{
				int context = 1;
				for (int i = 7; i >= 0; i--)
				{
			...
			public void EncodeMatched(compress.lzma.Compression.RangeCoder.Encoder rangeEncoder, byte matchByte, byte symbol) throws IOException
			{
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--)
			...
			public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
			{
				...
				if (matchMode)
				{
					for (; i >= 0; i--)
					{
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += compress.lzma.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit)
						{
							i--;
				...
				{
					int bit = (symbol >> i) & 1;
					price += compress.lzma.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
	...
	class LenEncoder
	{
		...
		{
			compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(_choice);
		...
		public void Encode(compress.lzma.Compression.RangeCoder.Encoder rangeEncoder, int symbol, int posState) throws IOException
		{
		...
		{
			int a0 = compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_choice[0]);
			int a1 = compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_choice[0]);
			int b0 = a1 + compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_choice[1]);
			int b1 = a1 + compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_choice[1]);
			int i = 0;
	...
	compress.lzma.Compression.LZ.BinTree _matchFinder = null;
	compress.lzma.Compression.RangeCoder.Encoder _rangeEncoder = new compress.lzma.Compression.RangeCoder.Encoder();
	...
	void Create()
	{
		...
		{
			compress.lzma.Compression.LZ.BinTree bt = new compress.lzma.Compression.LZ.BinTree();
			int numHashBytes = 4;
	...
	void Init()
	{
		...
		compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(_isMatch);
		compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(_isRep0Long);
		compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(_isRep);
		compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(_isRepG0);
		compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(_isRepG1);
		compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(_isRepG2);
		compress.lzma.Compression.RangeCoder.Encoder.InitBitModels(_posEncoders);
	...
	{
		return compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]) +
				compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetPureRepPrice(int repIndex, int state, int posState)
	{
		...
		if (repIndex == 0)
		{
			price = compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]);
			price += compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		}
		else
		{
			price = compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isRepG1[state]);
			else
			{
				price += compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isRepG1[state]);
				price += compress.lzma.Compression.RangeCoder.Encoder.GetPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
				_literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!Base.StateIsCharState(_state), matchByte, currentByte);
		_optimum[1].MakeAsChar();

		int matchPrice = compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isRep[_state]);
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice +
				compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
			...
			matchPrice = curPrice + compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price +
						compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
					{
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice =
								...
								compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						...
						int nextMatchPrice = curAndLenCharPrice + compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + compress.lzma.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
			...
			if (newLen >= startLen)
			{
				normalMatchPrice = matchPrice + compress.lzma.Compression.RangeCoder.Encoder.GetPrice0(_isRep[state]);
	...
	void FillDistancesPrices()
	{
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
		{
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << compress.lzma.Compression.RangeCoder.Encoder.kNumBitPriceShiftBits);
